...
int Request::computeEmergencyScore() const {
    ...
    // Checking request type and computing score accordingly
    if (type == "SUPPLY") {
        return emergencyLevel * 10 + std::min(amount, 50);
    } else if (type == "RESCUE") {
...
int Request::computeWorkloadContribution() const {
    ...
    // Checking request type and computing workload accordingly
    if (type == "SUPPLY") {
        return amount;
    } else if (type == "RESCUE") {
